...
#define portINITIAL_SYSTEM_PSW      (0x000008FFUL)

/* Context save area macros */
#define portCSA_FCX_MASK            (0x000FFFFFUL)
#define portINITIAL_LOWER_PCXI      (0x00300000UL) /* Set UL to upper and PIE to 1 in Register PXCI */
#define portINITIAL_UPPER_PCXI      (0x00200000UL) /* Set UL to lower and PIE to 1 in Register PXCI */
#define portNUM_WORDS_IN_CSA        (16)

#ifdef USE_INTERNAL_STM
 ...
 **********************************************************************************************************************/
/* Register defines */
static volatile uint32_t *const pxContextSrc = (uint32_t *)configCONTEXT_SRC;

#ifdef USE_INTERNAL_STM
static volatile uint32_t *const pxStm = (uint32_t *)configSTM;
static volatile uint32_t *const pxStmSrc = (uint32_t *)configSTM_SRC;
#endif
...
static inline void __attribute__((always_inline)) vPortLoadContext(uint8_t ucCallDepth);
static inline void __attribute__((always_inline)) vPortSaveContext(uint8_t ucCallDepth);
...
#endif
#ifdef TIMER_STM_H
#ifdef USE_TIMER_STM_APP
  TIMER_STM_Stop(configTIMER_HANDLER, configSTM_CMP_NR);
  TIMER_STM_Start(configTIMER_HANDLER, configSTM_CMP_NR, 1000000 / configTICK_RATE_HZ);
#endif
#endif
#ifdef TIMER_GTM_H
#ifdef USE_TIMER_GTM_APP
  TIMER_GTM_Stop(configTIMER_HANDLER);
  TIMER_GTM_SetTimeInterval(configTIMER_HANDLER, 1000000 / configTICK_RATE_HZ);
  TIMER_GTM_Start(configTIMER_HANDLER);
#endif
#endif
  vPortInitContextSrc();
  vPortStartFirstTask();
...
{
#ifdef USE_INTERNAL_STM
  pxStmSrc[0] &= ~(1 << portSRC_SRCR_SRE_OFF);      /* Disable STM Service request */
#endif
#ifdef TIMER_STM_H
#ifdef USE_TIMER_STM_APP
  TIMER_STM_Stop(configTIMER_HANDLER, configSTM_CMP_NR);
...
#endif
#endif
  pxContextSrc[0] &= ~(1 << portSRC_SRCR_SRE_OFF);  /* Disable Context Service request */
}
...
void vPortSystemContextHandler()
{
...
IFX_INTERRUPT(vPortSystemContextHandler, configCPU_NR, configCONTEXT_INTERRUPT_PRIORITY);
...
#ifdef TIMER_STM_H
#ifdef USE_TIMER_STM_APP
  TIMER_STM_SetNextInterval(configTIMER_HANDLER, configSTM_CMP_NR);
  TIMER_STM_ClearInterrupt(configTIMER_HANDLER, configSTM_CMP_NR);
#endif
#endif
#ifdef TIMER_GTM_H
#ifdef USE_TIMER_GTM_APP
  TIMER_GTM_ClearEvent(configTIMER_HANDLER);
#endif
#endif
...
#ifdef USE_INTERNAL_STM
IFX_INTERRUPT(vPortSystemTickHandler, configTIMER_NR, configTIMER_INTERRUPT_PRIORITY);
#endif

#ifdef USE_INTERNAL_STM
void vPortInitTickTimer()
{
  pxStm[portSTM_COMCON >> 2] = (0 << portSTM_CMCON_MSTART0_OFF) | (31 << portSTM_CMCON_MSIZE0_OFF);

  /* Interrupt output STMIRO selected */
  pxStm[portSTM_ICR >> 2] &= ~(1 << portSTM_ICR_CMP0OS_OFF);
  /* Set TOS and Priority of Service request */
  pxStmSrc[0] = ((configTIMER_NR > 0 ? configTIMER_NR + 1 : configTIMER_NR) << portSRC_SRCR_TOS_OFF) |
                ((configTIMER_INTERRUPT_PRIORITY) << portSRC_SRCR_SRPN_OFF);
  /* Enable STM Service request */
  pxStmSrc[0] |= (1 << portSRC_SRCR_SRE_OFF);
  pxStm[portSTM_CMP0 >> 2] = pxStm[portSTM_TIM0 >> 2];
  /* Compare Registor CMP0 Interrupt Request Flag is cleared. */
  pxStm[portSTM_ISCR >> 2] |= (1 << portSTM_ISCR_CMP0IRR_OFF);
...
void vPortInitContextSrc()
{
  /* Set TOS and Priority of Service request */
  pxContextSrc[0] =
      ((configCPU_NR > 0 ? configCPU_NR + 1 : configCPU_NR) << portSRC_SRCR_TOS_OFF) |
      ((configCONTEXT_INTERRUPT_PRIORITY) << portSRC_SRCR_SRPN_OFF);

  /* Enable Context Service request */
...
void vPortLoadContext(uint8_t ucCallDepth)
{
...
void vPortSaveContext(uint8_t ucCallDepth)
{
  uint32_t **ppxTopOfStack;
